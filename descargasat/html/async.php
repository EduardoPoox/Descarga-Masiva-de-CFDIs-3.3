...
// BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB
  ...
  // 
    $nombreDeArchivoDeDescargas = 'descargasat.zip';
    if(file_exists($nombreDeArchivoDeDescargas)){
      unlink($nombreDeArchivoDeDescargas);
    }

    $zip = new ZipArchive;
    $zip->open($nombreDeArchivoDeDescargas, ZipArchive::CREATE);
  // foreach (glob('../descargas/*.xml') as $file) 
  // ejemplo: $images = glob("images/{*.jpg,*.gif,*.png}", GLOB_BRACE);
  //  foreach (glob('../descargas/{*.xml,*.pdf}') as $file) 
    foreach (glob('../descargas/*.xml') as $file) 
    {
        $zip->addFile($file);
    }
    foreach (glob('../descargas/*.pdf') as $file) 
    {
        $zip->addFile($file);
    }    
    $zip->close();
   ...
   // $files = glob('../descargas/*.xml'); //obtenemos todos los nombres de los archivos
    $files = glob('../descargas/*.xml'); //obtenemos todos los nombres de los archivos    
    foreach($files as $file){
       if(is_file($file))
          unlink($file); 
    }
    $files = glob('../descargas/*.pdf'); //obtenemos todos los nombres de los archivos    
    foreach($files as $file){
       if(is_file($file))
          unlink($file); 
    }
